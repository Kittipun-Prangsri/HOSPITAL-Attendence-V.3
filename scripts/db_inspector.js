...
async function inspect() {
  try {
    ...
    console.log(`--- Inspecting today's (${today}) scans in hikvision for KHH00108 ---`);
    const [scans] = await hosofficePool.query(`
      SELECT EmployeeID, AccessDate, AccessTime, Direction, DeviceName, is_notified
      FROM hikvision
      WHERE AccessDate = ? AND EmployeeID = 'KHH00108'
      ORDER BY AccessTime DESC
      LIMIT 10
    `, [today]);
    console.table(scans);

    console.log(`--- Checking if any unprocessed scans exist today (${today}) ---`);
    const [unprocessed] = await hosofficePool.query(`
      SELECT EmployeeID, AccessDate, AccessTime, is_notified
      FROM hikvision
      WHERE AccessDate = ? AND (is_notified = 0 OR is_notified IS NULL)
      LIMIT 5
    `, [today]);
